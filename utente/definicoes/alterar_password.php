<?php
    ...
    //Caso o utente tenha inserido todos os campos obrigatórios e tenha clicado no botão para alterar a palavra-passe
    if(isset($_POST['mudar_password'])){
        ...
        //Query para ir buscar a palavra-passe do utente
        ...
        if(password_verify($pass_atual, $row["PASSWORD"])) {
            if($pass_nova == $pass_confirmacao){    //Caso a nova palavra-passe e a de confirmação sejam iguais
                ...
                if($result && $result1){    //Caso não tenha ocorrido nenhum erro
                    ...
                    header("Location: /projeto/utente/definicoes/?success= Palavra-passe alterada com sucesso!");    //Mensagem de sucesso
                    exit();
                }
                else{   //Caso contrário
                ...
                header("Location: /projeto/utente/definicoes/?error2= As Palavras-passe não são iguais!");    //Mensagem de erro
                exit();
            }
        }
        else {  //Caso contrário
            header("Location: /projeto/utente/definicoes/?error1= Palavra-passe atual errada!");    //Mensagem de erro
            exit();
        }
    }
    else {
