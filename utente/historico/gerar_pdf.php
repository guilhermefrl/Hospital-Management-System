<?php
    ...
    if(isset($_POST['gerar_pdf'])){     //Caso o utente tenha clicado no botão para gerar o pdf
        $ID_CONSULTA = $_POST['id'];

        //Query para ir buscar todos os dados da consulta
        $sql = "SELECT DISTINCT utilizador.ID_UTILIZADOR, utilizador.NOME_COMPLETO, (DATE_FORMAT(utente.DATA_NASCIMENTO, '%d/%m/%Y')) AS DATA_NASCIMENTO, utente.TELEMOVEL, consulta.DATA, consulta.DIAGNOSTICO, consulta.OBSERVACOES, consulta.PRECO FROM utilizador, utente, pedido_consulta, consulta WHERE consulta.ID_CONSULTA=$ID_CONSULTA AND consulta.ID_PEDIDO=pedido_consulta.ID_PEDIDO AND pedido_consulta.ID_UTENTE=utente.ID_UTENTE AND utente.ID_UTILIZADOR=utilizador.ID_UTILIZADOR";
        ...
        //Query para ir buscar os dados do médico que realizou a consulta
        ...
        if(mysqli_num_rows($result2) === 0){    //Caso não tenham sido receitados medicamentos
            ...
            $pdf->Output('Consulta_'.$ID_CONSULTA.'.pdf', 'I');     //Gerar o pdf
