<?php
    ...
    if(isset($_POST['alterar_dados'])){ //Caso o administrador tenha alterado os seus dados e tenha clicado no botão para alterar os dados
        ...
        if((mysqli_num_rows($result2) > 0 && $row2['ID_UTILIZADOR'] != $ID_Utilizador) || (mysqli_num_rows($result3) > 0 && $row3['ID_UTILIZADOR'] != $ID_Utilizador)){
            header("Location: /projeto/admin/definicoes/?error3= Já existe este nome de utilizador ou email!");    //Mensagem de erro
            exit();
        }
        else{   //Caso contrário
            ...
            if($result1){    //Caso não tenha ocorrido nenhum erro
                ...
                header("Location: /projeto/admin/definicoes/?success1= Dados alterados com sucesso!");    //Mensagem de sucesso
                exit();
            }
            else{   //Caso contrário
