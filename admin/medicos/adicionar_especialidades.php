...
if (isset($_SESSION['USERNAME']) && isset($_SESSION['ID_UTENTE']) === FALSE && isset($_SESSION['ID_MEDICO']) === FALSE) {
    ...
    //Query para ir buscar os dados de todas as especialidades
...
<html>
    ...
    <body id="body">
        ...
        <!--------------------------------------------------------
        // Pop-Up para adicionar uma nova especialidade
        --------------------------------------------------------->
        <div class="modal fade" id="Modal_adicionar_especialidade" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <form action="adicionar1.php" method="POST">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            ...
                            <button type="submit" name="adicionar_especialidade" class="btn btn-primary">Adicionar</button>
                        </div>
                    </div>
                </form>
            </div>
        ...
        <!-------------------------------------------------------------------
        // Pop-Up a informar que a especialidade foi adicionada com sucesso
        -------------------------------------------------------------------->
        ...
        <section class="shadow-sm mx-auto w-75 shadow clearfix my-5 rounded bg-white p-5 resize">
            ...
            <h4><i class="fa fa-user-md" aria-hidden="true"></i> Adicionar Especialidade</h4>
            <hr class="my-4">

                <div class="form-row">
                    <button class="btn btn-primary adicionar_especialidade_btn" style="margin-left: 2%;" type="button">Adicionar</button>
                </div>
                </br>
                    ...
                    <table id="myTable" class="table table-striped table-bordered" style="width:100%">
                        ...
                        <?php
                            while($row = mysqli_fetch_array($result)){  //Fazer para todas as linhas do resultado da query
                                echo '<tr>';
                                    echo '<td>'.$row["ID_ESPECIALIDADE"].'</td>';   //Mostrar o ID da especialidade
                                    echo '<td>'.$row["NOME"].'</td>';               //Mostrar o nome da especialidade
                                echo '</tr>';
                            }
                        ?>
                    </table>
...
<script>
    $('.adicionar_especialidade_btn').on('click', function(){   //Caso o botão para adicionar uma nova especialidade tenha sido clicado
        $('#Modal_adicionar_especialidade').modal('show');      //Mostrar o pop-up para adicionar uma nova especialidade
    });

    $("#Modal_adicionar_especialidade").on('hidden.bs.modal', function () { //Caso o pop-up para adicionar uma especialidade seja fechado
    ...
    $("#myModal").on('hidden.bs.modal', function () {          //Caso o pop-up seja fechado
        window.location.href = '/projeto/admin/medicos/adicionar_especialidades.php';       //Atualizar a página
    });
    ...
    $(document).ready( function () {        //Configuração da tabela
        $('#myTable').DataTable({
            ...
            "zeroRecords":    "Não foi encontrada nenhuma especialidade.",
            "info":           "_END_ especialidades no total de _TOTAL_ especialidades",
            "infoEmpty":      "",
