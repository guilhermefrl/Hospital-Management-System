...
if (isset($_SESSION['USERNAME']) && isset($_SESSION['ID_UTENTE']) === FALSE && isset($_SESSION['ID_MEDICO']) === FALSE) {
    ...
    $sql = "SELECT utilizador.NOME_COMPLETO, utilizador.EMAIL, (especialidades.NOME) AS ESPECIALIDADE, medico.EXPERIENCIA FROM utilizador, medico, especialidades WHERE utilizador.TIPO=2 AND utilizador.ID_UTILIZADOR=medico.ID_UTILIZADOR AND medico.ESPECIALIDADE=especialidades.ID_ESPECIALIDADE";
...
<html>
    ...
    <body id="body">
        ...
        <!--------------------------------------------------------
        // Pop-Up para adicionar um médico
        --------------------------------------------------------->
        <div class="modal fade" id="Modal_adicionar_medico" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="form-row">
                                <div class="col-md-12 mb-3">
                                    ...
                                    <select name="especialidade" class="form-control">
                                        ...
                                        <option value=<?php echo $row1['ID_ESPECIALIDADE'];?>><?php echo $row1['NOME'];?></option>
                                        <?php endwhile;?>
                                    </select>
                                </div>
                            </div>
                            <div class="form-row">
                                <div class="col-md-4 mb-3">
                                    ...
                                    <input type="number" step="0.01" class="form-control" name="experiencia" required>
                                </div>
                            </div>
                        </div>
        ...
        <!------------------------------------------------------------
        // Pop-Up a informar que o médico foi adicionado com sucesso
        ------------------------------------------------------------->
        ...
        <section class="shadow-sm mx-auto w-75 shadow clearfix my-5 rounded bg-white p-5 resize">
            ...
            <h4><i class="fa fa-user-md" aria-hidden="true"></i> Adicionar Médicos</h4>
            <hr class="my-4">

                <div class="form-row">
                    <button class="btn btn-primary adicionar_medico_btn" style="margin-left: 2%;" type="button">Adicionar</button>
                </div>
                </br>
...
<script>
    $('.adicionar_medico_btn').on('click', function(){   //Caso o botão para adicionar um novo médico tenha sido clicado
        $('#Modal_adicionar_medico').modal('show');      //Mostrar o pop-up para adicionar um novo médico
    });

    $("#Modal_adicionar_medico").on('hidden.bs.modal', function () {      //Caso o pop-up para adicionar um médico seja fechado
    ...
    $("#myModal").on('hidden.bs.modal', function () {          //Caso o pop-up seja fechado
        window.location.href = '/projeto/admin/medicos/adicionar_medicos.php';       //Atualizar a página
    });
