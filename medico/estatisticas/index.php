...
if (isset($_SESSION['ID_MEDICO'])) {        //Caso o médico tenha feito login
    ...
    FROM consulta, pedido_consulta, medico WHERE YEAR(DATA) = '$timestamp' AND consulta.ID_PEDIDO=pedido_consulta.ID_PEDIDO AND pedido_consulta.ID_MEDICO=medico.ID_MEDICO AND medico.ID_MEDICO=$id";
    ...
    SUM(CASE MONTH(DATA) WHEN 11 THEN 1 ELSE 0 END) AS '11',
    SUM(CASE MONTH(DATA) WHEN 12 THEN 1 ELSE 0 END) AS '12'
    FROM consulta, pedido_consulta, medico WHERE YEAR(DATA) = '$timestamp1' AND consulta.ID_PEDIDO=pedido_consulta.ID_PEDIDO AND pedido_consulta.ID_MEDICO=medico.ID_MEDICO AND medico.ID_MEDICO=$id";
    ...
    //Query para contar o número de utentes do médico em cada distrito
    $sql8="SELECT distritos.CODIGO_DISTRITO, COUNT(DISTINCT utente.ID_UTENTE) AS D_TOTAL FROM distritos LEFT JOIN todos_cp ON (todos_cp.CODIGO_DISTRITO=distritos.CODIGO_DISTRITO) INNER JOIN utente ON (utente.CP4=todos_cp.CP4 AND utente.CP3=todos_cp.CP3) INNER JOIN pedido_consulta ON (pedido_consulta.ID_UTENTE=utente.ID_UTENTE AND pedido_consulta.ID_MEDICO=$id) INNER JOIN consulta ON (pedido_consulta.ID_PEDIDO=consulta.ID_PEDIDO) GROUP BY distritos.CODIGO_DISTRITO";
    ...
    data: {
        labels: [<?php
                while($row8 = mysqli_fetch_array($result8)){ 
                    $i=$row8['CODIGO_DISTRITO'];
                    $a=["Aveiro", "Beja", "Braga", "Bragança", "Castelo Branco", "Coimbra", "Évora", "Faro", "Guarda", "Leiria", "Lisboa", "Portalegre", "Porto", "Santarém", "Setúbal", "Viana do Castelo", "Vila Real", "Viseu", "Ilha da Madeira", "Ilha de Porto Santo", "Ilha de Santa Maria", "Ilha de São Miguel", "Ilha Terceira", "Ilha da Graciosa", "Ilha de São Jorge"];
                    echo '"'.$a[($i)-1].'",'; 
                }
            ?>],
      datasets: [
        {
          label: "Distritos dos utentes",
          backgroundColor: ["#00b4d8", "#00f5d4", "#f9c74f", "#B9E3C6", "#A3E7FC", "#F64740", "#477998", "#C4D6B0", "#77A6B6", "#B3D89C", "#D0EFB1","#ffce56", "#B9E3C6","#59C9A5","#7FEFBD","#FFFD98", "#26C485", "#A3E7FC", "#F64740", "#477998", "#C4D6B0", "#77A6B6", "#B3D89C", "#D0EFB1","#ffce56", "#B9E3C6","#59C9A5","#7FEFBD","#FFFD98", "#26C485", "#A3E7FC", "#F64740", "#477998", "#C4D6B0", "#77A6B6", "#B3D89C"],
          data: [ <?php
